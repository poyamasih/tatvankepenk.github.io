...
// Import the real EnhancedDrawer with a prefix
import 'package:tatvan_kepenk/widgets/enhanced_drawer.dart' as drawer;
...
class _HomePageState extends State<HomePage>
  ...
  void initState() {
    ...
    // Add safety timer to ensure UI is displayed even if there's an error
  ...
  Future<void> _initializeData() async {
    try {
      ...
      // Try to sync data - continue even if there's an error
      try {
        await _syncDataIfNeeded();
      } catch (syncError) {
        debugPrint(
  ...
  Widget _buildMainContent(double screenHeight) {
    return Scaffold(
      ...
      // Use the proper EnhancedDrawer with drawer prefix
      endDrawer: drawer.EnhancedDrawer(
        currentPageIndex: _getSafePageIndex(_currentPageIndex),
        onPageChanged: (index) {
          if (index != _currentPageIndex) {
            _navigateToPage(index);
